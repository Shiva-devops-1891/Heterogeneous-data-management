	...
	<body onload="init()" id="page-top" data-spy="scroll"
		...
		<div id="home">
			...
			<div class="login-form section text-center">
                ...
                String mac = (String) httpSession.getAttribute("mac");
                
                String data = "http://localhost:8888/Deduplicationn/bind/"+file;%>
						<object data="<%=data %>" width="300" height="200">
							Not supported
						</object>
						...
						<center>
							<input type="submit" value="Generate hash" id="mac"
								onclick="myFunction()">
						</center>
						<center>
						<form action="deduplicate" method="post">
						<input type="hidden" name="mac" id="mac" value="<%=mac%>">
							<input style="display: none;" type="submit"
								value="Deduplication Check" id="deduplication">
						</center>
						</form>
					</div>
...
function dedup() {
	...
	$.ajax( {
		url : "deduplicate", //<-- PHP Script where you want to post your data
		type : "POST",
		data : post,
		success : function(data) {
			console.log(data);
			if(data === "no"){
				alert("No duplicate file found on this server check in another Server")
				window.location.replace("globalserver.jsp")
			}
...
function showPage() {
	...
	document.getElementById("deduplication").style.display = "block";
